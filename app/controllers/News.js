...
const savenewsImg = async (req, res) => {
  try {
    ...
    if (req.files && req.files.image) {
      ...
      // Subir la imagen a ImageKit, en la carpeta 'Noticias'
      imageUrl = await uploadToImageKit(image.buffer, filename, image.mimetype, 'Noticias');
    }
    ...
    const newsSchema = new news({
      ...
      link,
      tipo_noticia,
      imagen: imageUrl,  // URL de la imagen (subida a ImageKit)
